...
pub(crate) fn validate_replace_tags(from: &str, to: &str) -> Result<(), String> {
...
pub(crate) fn validate_remove_tag(target: &str) -> Result<(), String> {
...
/// 1 メモの frontmatter tags を置換。変更なしなら `None`。
pub(crate) fn replace_tags_in_note_content(content: &str, from: &str, to: &str) -> Option<String> {
    let from_trim = from.trim();
    let to_trim = to.trim();
    ...
    let new_content = rebuild_note_frontmatter_tags(content, next)?;
...
/// 1 メモの frontmatter tags から除去。変更なしなら `None`。
pub(crate) fn remove_tag_from_note_content(content: &str, target: &str) -> Option<String> {
    let target = target.trim();
    let cap = note_frontmatter_regex().captures(content)?;
    let fm_raw = cap.get(1)?.as_str();
    let tags = parse_tags_from_fm_inner(fm_raw);
...
pub fn replace_tag_globally(
    ...
    validate_replace_tags(from_trim, to_trim)?;
    ...
    for path in &paths {
        ...
        let Some(new_content) = replace_tags_in_note_content(&content, from_trim, to_trim) else {
            continue;
        };
...
pub fn remove_tag_globally(app: tauri::AppHandle, tag: String) -> Result<ReplaceTagGloballyResult, String> {
    ...
    for path in &paths {
        ...
        let Some(new_content) = remove_tag_from_note_content(&content, target) else {
            continue;
        };
        fs::write(path, new_content).map_err(|e| io(app_error::FILE_WRITE_FAILED, e))?;
        changed_paths.push(path.to_string_lossy().into_owned());
...
mod tests {
    ...
    fn note_with_tags(tags: &str) -> String {
        format!("---\ntags: {tags}\n---\n\n# Title\n")
    }

    #[test]
    fn validate_replace_tags_rejects_empty_from() {
        assert_eq!(
            validate_replace_tags("", "work").unwrap_err(),
            app_error::TAG_REPLACE_FROM_EMPTY
        );
    }

    #[test]
    fn validate_replace_tags_rejects_empty_to() {
        assert_eq!(
            validate_replace_tags("work", "  ").unwrap_err(),
            app_error::TAG_REPLACE_TO_EMPTY
        );
    }

    #[test]
    fn validate_replace_tags_rejects_reserved_tags() {
        assert_eq!(
            validate_replace_tags("_builtin", "work").unwrap_err(),
            app_error::TAG_REPLACE_FROM_RESERVED
        );
        assert_eq!(
            validate_replace_tags("work", "#_builtin").unwrap_err(),
            app_error::TAG_REPLACE_TO_RESERVED
        );
    }

    #[test]
    fn validate_replace_tags_accepts_normal_tags() {
        assert!(validate_replace_tags("work", "project").is_ok());
    }

    #[test]
    fn validate_remove_tag_rejects_inbox_and_reserved() {
        assert_eq!(
            validate_remove_tag("inbox").unwrap_err(),
            app_error::TAG_REMOVE_INBOX
        );
        assert_eq!(
            validate_remove_tag("_builtin").unwrap_err(),
            app_error::TAG_REMOVE_RESERVED
        );
        assert_eq!(
            validate_remove_tag("").unwrap_err(),
            app_error::TAG_REMOVE_EMPTY
        );
    }

    #[test]
    fn replace_tags_in_note_content_renames_case_insensitive() {
        let content = note_with_tags("Work, draft");
        let next = replace_tags_in_note_content(&content, "work", "project").unwrap();
        assert!(next.contains("tags: project, draft"));
    }

    #[test]
    fn replace_tags_in_note_content_returns_none_when_tag_missing() {
        let content = note_with_tags("draft");
        assert!(replace_tags_in_note_content(&content, "work", "project").is_none());
    }

    #[test]
    fn replace_tags_in_note_content_returns_none_without_frontmatter() {
        assert!(replace_tags_in_note_content("# Title\n", "a", "b").is_none());
    }

    #[test]
    fn remove_tag_from_note_content_removes_target() {
        let content = note_with_tags("work, draft");
        let next = remove_tag_from_note_content(&content, "WORK").unwrap();
        assert!(next.contains("tags: draft"));
    }

    #[test]
    fn remove_tag_from_note_content_returns_none_when_absent() {
        let content = note_with_tags("draft");
        assert!(remove_tag_from_note_content(&content, "work").is_none());
    }

    #[test]
    fn replace_to_inbox_when_only_tag_becomes_inbox_only() {
        let content = note_with_tags("work");
        let next = replace_tags_in_note_content(&content, "work", "inbox").unwrap();
        assert!(next.contains("tags: inbox"));
    }

    #[test]
    fn replace_to_inbox_with_other_tags_drops_inbox() {
        let content = note_with_tags("work, draft");
        let next = replace_tags_in_note_content(&content, "work", "inbox").unwrap();
        assert!(next.contains("tags: draft"));
        assert!(!next.contains("inbox"));
    }
}
